...
.. py:class:: ARModel(args, config: neural_lam.config.NeuralLAMConfig, datastore: neural_lam.datastore.BaseDatastore)
   ...
   .. py:method:: aggregate_and_plot_metrics(metrics_dict, prefix)
      ...
      :param metrics_dict: Dictionary mapping metric names to lists of tensors with
                           step-evals.
      :type metrics_dict: dict
      :param prefix: Prefix to use for logging (e.g. ``"val"`` or ``"test"``).
      :type prefix: str
   ...
   .. py:method:: all_gather_cat(tensor_to_gather)
      ...
      :param tensor_to_gather: Shape ``(d1, d2, ...)``, distributed over K ranks.
      :type tensor_to_gather: torch.Tensor

      :returns: Shape ``(K*d1, d2, ...)``.
      :rtype: torch.Tensor
   ...
   .. py:method:: common_step(batch)
      ...
      :param batch: Contains ``(init_states, target_states, forcing_features,
                    batch_times)`` where:
   ...
   .. py:method:: create_metric_log_dict(metric_tensor, prefix, metric_name)
      ...
      :param metric_tensor: Shape ``(pred_steps, d_f)``, metric values per time and variable.
      ...
      :returns: Dictionary with everything to log for the given metric.
      :rtype: dict
   ...
   .. py:method:: plot_examples(batch, n_examples, split, prediction=None)
      ...
      :param batch: Batch with data to plot corresponding forecasts for.
      :type batch: tuple
      :param n_examples: Number of forecasts to plot.
      :type n_examples: int
      :param split: Data split name (e.g. ``"test"``).
      :type split: str
      :param prediction: Shape ``(B, pred_steps, num_grid_nodes, d_f)``, existing
                         prediction. Generated from batch if None.
      :type prediction: torch.Tensor or None
   ...
   .. py:method:: predict_step(prev_state, prev_prev_state, forcing)
      ...
      :param prev_state: Shape ``(B, num_grid_nodes, feature_dim)``, state at time t.
      :type prev_state: torch.Tensor
      :param prev_prev_state: Shape ``(B, num_grid_nodes, feature_dim)``, state at time t-1.
      :type prev_prev_state: torch.Tensor
      :param forcing: Shape ``(B, num_grid_nodes, forcing_dim)``.
   ...
   .. py:method:: unroll_prediction(init_states, forcing_features, true_states)
      ...
      :param init_states: Shape ``(B, 2, num_grid_nodes, d_f)``.
      :type init_states: torch.Tensor
      :param forcing_features: Shape ``(B, pred_steps, num_grid_nodes, d_static_f)``.
      :type forcing_features: torch.Tensor
      :param true_states: Shape ``(B, pred_steps, num_grid_nodes, d_f)``.
